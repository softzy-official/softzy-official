...
const ProductInfo = ({ product }: ProductInfoProps) => {
  ...
  const maxAllowedToAdd = Math.max(0, (product.stockCount || 0) - currentCartQty);
  ...
  return (
    <div className="space-y-5 sm:space-y-6">
      ...
      {product.sizes && product.sizes.length > 0 && (
        <div className={`space-y-3 p-2 -mx-2 rounded-lg transition-colors ${sizeError ? 'bg-red-50' : ''}`}>
          <label className={`text-sm font-medium poppins ${sizeError ? 'text-red-600' : 'text-foreground'}`}>
      ...
      {product.inStock && product.stockCount && product.stockCount > 0 && (
        <div className="space-y-3">
          ...
          {maxAllowedToAdd === 0 && (
             <p className="text-sm text-orange-500 poppins">You have all available stock in your cart.</p>
          )}
        </div>
      )}
      ...
      <div className="flex flex-col sm:flex-row gap-3 pt-2">
        <button
          ...
          disabled={!product.inStock || !product.stockCount || product.stockCount === 0 || maxAllowedToAdd === 0}
        ...
        >
          {addedToCart ? (
            ...
            <>
              ...
              {!product.inStock || !product.stockCount || product.stockCount === 0 ? "Out of Stock" : "Add to Cart"}
            </>
          )}
        </button>
